...
| 보유·포트폴리오 | **내 보유 편집** + 밤사이 움직임 + 콕핏 |
| 관심 종목 | 실적 캘린더 |

선택은 URL 해시에 남아 북마크·새로고침에 견딘다 (`#events`).
**섹션은 숨겨질 뿐 지워지지 않는다** — 여러 섹션에서 체크한 종목이 배치 바구니에 함께 담긴다.

체크박스가 두 종류다. 섞이면 안 된다:

| 클래스 | 어디 | 무엇 |
|---|---|---|
| `.pick` | 검색·이벤트·시장 랭킹·**보유 표** | 분석 바구니 (배치 생성) |
| `.hpick` | 보유 뷰의 주요 기업 목록 | 보유에 담기 (파일 저장) |

보유 표의 종목은 눌러서 바로 분석 페이지로 갈 수도 있다 — 보유를 들여다보는 순간이
곧 "이 종목을 더 파봐야겠다"는 순간이라 같은 자리에 둔다.

## 2. 보유 현황 고치기 — 세 가지 길, 같은 파일

`portfolio/holdings.yaml` 이 유일한 진실의 원천이다 (CLAUDE.md).
어느 길로 고치든 **브로커에서 끌어오지 않는다** — 사람이 고른 것만 파일에 적는다.

| 길 | 어디 | 쓸 때 |
|---|---|---|
| 대시보드 `보유·포트폴리오` 뷰 | serve (8766) | 목록 체크·검색으로 담고 수량·평단만 채울 때 |
| 편집기 폼 | `python3 -m src.pipelines.editor` (8765) | 여러 줄을 한꺼번에 손볼 때 |
| YAML 직접 | 에디터 | 스크립트로 옮겨 적을 때 |

셋 다 `portfolio_io.save_holdings()` 를 지나거나 같은 규칙을 따른다:
**쓰기 전 `.yaml.bak` 백업 → 쓰고 다시 읽어 검증 → 실패하면 원복.**
보유가 깨지면 모든 계산이 오염되므로 깨진 파일을 남기느니 저장을 실패시킨다.

> `load_holdings` 는 형식 오류에 `Unavailable` 이 아니라 **`PortfolioError` 를 던진다.**
> 반환값만 검사하면 예외가 빠져나가 깨진 파일이 남는다 — 실제로 그렇게 새어 나갔던 자리다.
> `tests/test_save_holdings.py::TestRollback` 이 이걸 고정한다.

직접 YAML 을 고쳤다면 스냅샷이 안 남으므로 수익 기여도 계산이 밀린다.
편집기와 대시보드 저장은 스냅샷을 자동으로 남긴다.

대시보드에서 저장하면 파생 표(평가액·콕핏·신호)는 **백그라운드로 재생성**된다 —
저장 자체는 즉시 끝나고, 화면이 "잠시 후 새로고침하세요"라고 알린다.
...
`render/{dashboard,glossary}` · `portfolio_io.save_holdings` · `provenance.record`
